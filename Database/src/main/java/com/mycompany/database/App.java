...
public class App extends Application {
	...
	public void stop() {
		...
		runDBQuery("DELETE FROM SEMESTER", 'd'); // SEMESTERS
		for (Semester s : semesters) {
			runDBQuery( // run
					"Insert into Semester (semesterID, periods, years) values (" + // Set up command
							s.getID() + ", '" + // semesterID
							s.getPeriod() + "', " + // period
							s.getYear() + // year
					")", 'u');
		...
		runDBQuery("DELETE FROM FACULTY", 'd'); // FACULTY
		...
		runDBQuery("DELETE FROM STUDENT", 'd'); // STUDENTS
